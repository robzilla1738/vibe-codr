...
- Consolidated the engine, CLI, terminal UI, cloud runtime, and Electron
  desktop app into one canonical `robzilla1738/vibe-codr` repository with one
  product version, one tag, and one GitHub release.
- Replaced the historical multi-author graph with a clean single-owner source
  history while preserving local recovery bundles of the prior repositories.
...
- Added a first-class Sessions destination with Board and List views across
  every project and Chat.
- Added search, project/mode/status filters, sorting, persisted Active/Review/
...
- Strengthened dependency-aware orchestration with durable structured
  handoffs, task report retrieval, worktree isolation, validated structured
  child output, detached execution, child continuation, and bounded review and
  retry behavior.
- Hardened concurrency and lifecycle edges: adaptive tree-global provider
  limiting, per-child continuation serialization, per-file writer ownership,
  detached finalization bounds, and honest unverified/failed outcomes.
- Kept plans evidence-grounded and approval-gated, while accepted plans seed
  the same durable task spine used by goals and execution.

### Reliability

- Raised timeouts only for the two real Git/submodule transfer integration
  cases that can exceed Vitest's five-second default under coverage or loaded
  CI runners; behavior and assertions remain unchanged.
- Updated product, architecture, desktop, parity, verification, privacy,
  release, and website documentation for the canonical repository.
