...
# Desktop shell in this repository — engine host for NDJSON bridge
...
- **Subsystem invariants (don't regress these — each has a test):**
  ...
  **Bare image paths auto-attach** via `parseBareImagePaths` (handles shell-escaped
  ...
  - **Skill invocation control (Claude Code / VS Code parity):** SKILL.md may set
    `disable-model-invocation: true` (user-only — omitted from progressive
    disclosure; `use_skill` errors; still runnable via `/name` or `/skill name`)
    or `user-invocable: false` (hidden from `/` menu; model may still load).
    Plan-mode skill doctrine is informational; plan-mode `use_skill` prefixes a
