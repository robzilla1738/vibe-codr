...
| Durable Needs-your-Mac requests | **Verified** | The production `macos` tool delegates open URL/path/application and reveal-path actions through the durable external-capability boundary; it never grants the cloud runtime local shell access. The engine returns approved results or denial errors to the live caller and retains remotely resolved requests for one-time consumption after handoff/restart. Focused tests cover tool delegation, live approval/denial, restored pending state, late resolution, and exactly-once return. |
