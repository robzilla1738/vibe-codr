...
const assistant = (id: number, text: string): Block => ({
  kind: "assistant", id, text, streaming: false, gap: false, timestamp: id,
});
...
describe("groupTranscriptItems", () => {
  it("keeps commentary, reasoning, tools, and final text first-class", () => {
    ...
    expect(result.map((item) => item.block.id)).toEqual([1, 2, 3, 4, 5, 6]);
  });
  ...
  it("preserves event order when the visible window starts on commentary", () => {
    ...
    expect(groupTranscriptItems(items, 2).map((item) => item.block.id)).toEqual([3, 4, 5, 6]);
  });
  ...
  it("does not hide narration emitted before the first model action", () => {
    ...
    expect(result.map((item) => item.block.id)).toEqual([1, 2, 3, 4]);
  });
