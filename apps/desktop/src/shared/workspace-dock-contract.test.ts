 ...
 * Session / Changes / Git / Browser / Terminal / Jobs / Files — no Local+Files duplicate, no
...
describe("workspace dock design contract", () => {
  ...
  const browserSource = readFileSync(join(process.cwd(), "src/renderer/panels/BrowserPanel.tsx"), "utf8");
  const styles = readFileSync(join(process.cwd(), "src/renderer/styles.css"), "utf8");

  it("exposes Session, Changes, Git, Browser, Terminal, Jobs, Files rows", () => {
    ...
    expect(source).toContain('ariaLabel="Open web browser"');
    ...
    for (const target of ["session", "changes", "git", "browser", "terminal", "jobs", "files"]) {
  ...
  it("opens every tool in one structural edge-attached activity sidebar", () => {
    ...
    expect(appSource).toContain("label={`Resize $" + "{renderedEndPanel} sidebar`}");
    ...
    for (const label of ["Session", "Changes", "Git", "Browser", "Terminal", "Jobs"]) {
    ...
    expect(terminalSource).toContain("requestUrlOpen(uri, event)");
    expect(terminalSource).toContain("letterSpacing: 0");
  });

  it("uses equal switcher columns and one shared header across every view", () => {
    expect(styles).toMatch(/\.activity-sidebar-tabs\s*\{[\s\S]*?grid-template-columns:\s*repeat\(6, minmax\(0, 1fr\)\);/);
    expect(headerSource).toContain('className="activity-panel-header sidebar-heading-row"');
    for (const panel of [inspectorSource, changesSource, gitSource, terminalSource, jobsSource, browserSource]) {
  ...
  it("keeps compact dock and drawer controls outside Electron drag regions", () => {
    ...
    expect(styles).toMatch(/@media \(max-width: 960px\)[\s\S]*?grid-template-columns:\s*repeat\(7, minmax\(0, 1fr\)\);/);
