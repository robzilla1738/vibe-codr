...
const rpcHandlers: Record<string, (params?: Record<string, unknown>) => unknown> = {
  ...
  readTrace: () => ({
    ...
    runId: "run-preview",
    events: [
      { schemaVersion: 1, runId: "run-preview", seq: 1, at: now - 12 * MIN, type: "session-start", sessionId: SID, model: "anthropic/claude-4.6-opus", mode: "execute" },
      { schemaVersion: 1, runId: "run-preview", seq: 2, at: now - 10 * MIN, type: "tool-call-started", sessionId: SID, toolCallId: "tool-preview", toolName: "edit" },
      { schemaVersion: 1, runId: "run-preview", seq: 3, at: now - 3 * MIN, type: "verify-finished", sessionId: SID, flags: { ok: true } },
      { schemaVersion: 1, runId: "run-preview", seq: 4, at: now - 2 * MIN, type: "engine-idle", sessionId: SID, gate: "green" },
    ],
    corruptions: [],
    lastSeq: 4,
    nextAfterSeq: null,
...
const mock = {
  ...
  cancelLocalRuntimeLaunch: async () => ({ ok: true as const, cancelled: false }),
  onLocalRuntimeLaunchQueue: () => () => undefined,
  onLocalRuntimeNotificationActivation: () => () => undefined,
  onFatal: () => () => undefined,
  ...
  browserLoad: async () => undefined,
  browserSetBounds: () => undefined,
  browserSetVisible: () => undefined,
  browserCommand: () => undefined,
  onBrowserState: () => () => undefined,
  onBrowserFocusAddress: () => () => undefined,
...
const REQUIRED_VIBE_KEYS = [
  ...
  "ensureChatsDir", "openExternal", "browserLoad", "browserSetBounds", "browserSetVisible", "browserCommand", "onBrowserState", "onBrowserFocusAddress", "showItem", "readTextFile", "composeInEditor",
