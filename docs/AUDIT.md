...
24. DOCUMENTED -- proactive recall re-triggering on topic shift requires tracking embedding distance between the first prompt and subsequent prompts, which adds an embedding call per prompt (cost). The current one-shot design is intentional: recall fires once at session start when the first prompt + goal are the strongest signal. A future enhancement could add a per-prompt embedding budget for re-triggering. No code change needed for the audit -- the current behavior is correct, just conservative.

25. DOCUMENTED -- session-transcript recall is a linear scan over `history.jsonl` files, which is fine for typical session counts (< 100). Indexing session digests into the semantic store would make it an ANN lookup, but adds complexity (index lifecycle, digest embedding cost). The linear scan is bounded by the number of sessions, not their size, and is mtime-cached. A future enhancement: embed digests at finalize into the semantic store.
...
27. DOCUMENTED -- saved facts accumulate without expiry. A recency-weighted fusion (like the `freshnessBoost` from searchcore.ts applied to web results) would down-rank stale facts. The `freshnessBoost` function already exists for web search; applying it to memory requires a date metadata on each fact (the dated file name provides this). A future enhancement: apply `freshnessBoost` to memory hits based on the dated file name.
