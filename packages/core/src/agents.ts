...
import type { AgentCapability, Mode } from "@vibe/shared";
...
export interface NamedAgent {
  ...
  system?: string;
  /** Persona is voice/working style; capabilities are enforceable routing data. */
  persona?: string;
  capabilities?: AgentCapability[];
  inputArtifact?: string;
  outputArtifact?: string;
  /** A required final proof/tool/result contract for this worker. */
...
function parseCapabilities(value: string | undefined): AgentCapability[] | undefined {
  ...
  const values = parseToolList(value)?.filter((item): item is AgentCapability =>
    allowed.has(item as AgentCapability),
  );
  ...
  const defs: NamedAgent[] = [
    {
      ...
      mode: "plan",
      persona: "curious codebase cartographer",
      capabilities: ["research"],
      outputArtifact: "A path-and-line evidence map with a direct answer.",
      system:
    ...
    },
    {
      ...
      mode: "execute",
      persona: "careful production implementer",
      capabilities: ["research", "code", "shell"],
      outputArtifact: "Implemented code plus verification evidence.",
      completionRequirement:
        "Run at least one relevant verification command and report its outcome.",
      completionTool: "run_check",
      system:
    ...
    },
    {
      ...
      mode: "plan",
      persona: "pessimistic adversarial reviewer",
      capabilities: ["research", "review"],
      outputArtifact: "Severity-ordered findings or exactly REVIEW-CLEAN.",
      system:
    ...
    },
    {
      ...
      mode: "execute",
      persona: "evidence-driven test author",
      capabilities: ["research", "test", "code", "shell"],
      outputArtifact: "Executable tests and their pass/fail result.",
      completionRequirement: "Run the authored tests and report the real result.",
      completionTool: "run_check",
      system:
    ...
    for await (const file of glob.scan({ cwd: dir, absolute: true })) {
      ...
      try {
        ...
        agents.set(name, {
          ...
          ...(frontmatter.input_artifact ? { inputArtifact: frontmatter.input_artifact } : {}),
          ...(frontmatter.output_artifact ? { outputArtifact: frontmatter.output_artifact } : {}),
          ...(frontmatter.completion_requirement
            ? { completionRequirement: frontmatter.completion_requirement }
            : {}),
          ...(frontmatter.completion_tool ? { completionTool: frontmatter.completion_tool } : {}),
          ...(body ? { system: body } : {}),
        });
      } catch {
...
export function serializeAgent(agent: NamedAgent): string {
  ...
  if (agent.persona) fm.push(`persona: ${agent.persona}`);
  if (agent.capabilities?.length) fm.push(`capabilities: ${agent.capabilities.join(", ")}`);
  if (agent.inputArtifact) fm.push(`input_artifact: ${agent.inputArtifact}`);
  if (agent.outputArtifact) fm.push(`output_artifact: ${agent.outputArtifact}`);
  if (agent.completionRequirement)
    fm.push(`completion_requirement: ${agent.completionRequirement}`);
  if (agent.completionTool) fm.push(`completion_tool: ${agent.completionTool}`);
  return `---\n${fm.join("\n")}\n---\n\n${(agent.system ?? "").trim()}\n`;
}
