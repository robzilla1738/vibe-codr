...
test("Engine planning: present_plan persists the plan + plan→execute injects a handoff", async () => {
  ...
  const pendingSnapshot = engine.snapshot();
  expect(pendingSnapshot.planState).toMatchObject({
    status: "pending",
    plan: "1. Refactor the loader\n2. Add tests",
  });
  const sessionId = pendingSnapshot.sessionId;
  ...
  await engine.whenIdle();
  expect(engine.snapshot().planState?.status).toBe("inactive");
  engine.send({ type: "shutdown" });
  await collector;
...
test("Engine structured question blocks the tool until a typed UI answer arrives", async () => {
  const cwd = mkdtempSync(join(tmpdir(), "vibe-engine-question-"));
  const steps = [
    stream([
      { type: "stream-start", warnings: [] },
      {
  ...
  const collector = (async () => {
    for await (const e of engine.events()) {
      ...
      if (e.type === "question-request") {
        engine.send({ type: "resolve-question", id: e.question.id, answers: ["Safe"] });
      }
    }
  })();
  engine.send({ type: "submit-prompt", text: "choose an approach" });
  await engine.whenIdle();
  expect(seen.some((event) => event.type === "question-request")).toBe(true);
  expect(
    seen.some((event) => event.type === "question-settled" && event.reason === "answered"),
  ).toBe(true);
  engine.send({ type: "shutdown" });
  await collector;
});
