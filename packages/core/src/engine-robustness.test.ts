...
test("recovers from a non-unique edit by retrying with replaceAll", async () => {
  ...
  const d = drive(
    [
      ...
      step(
        toolCall("c2", "edit", { path: "f.txt", oldString: "x", newString: "y", replaceAll: true }),
      ), // fixed
  ...
  const finishes = d.events.filter(
...
test("a failing shell command is surfaced as an error, turn still completes", async () => {
  ...
  const d = drive(
    [
      step(toolCall("c1", "bash", { command: "exit 7" })),
      textStep("The command failed with exit 7."),
    ],
    cwd,
  );
  await run(d, "run the thing");
  const fin = d.events.find(
    (e): e is Extract<UIEvent, { type: "tool-call-finished" }> => e.type === "tool-call-finished",
  );
...
test("malformed tool args produce a clear error, not a crash", async () => {
  ...
  const d = drive(
    [
      step(toolCall("c1", "edit", { path: "f.txt" })),
      textStep("I need to specify what to change."),
    ],
    cwd,
  );
...
test("a realistic grep -> read -> edit workflow on a real file", async () => {
  ...
  expect(await Bun.file(join(cwd, "app.ts")).text()).toBe(
    "const DONE = 1;\nexport const x = DONE;\n",
  );
  // All three tools ran in order without error.
  const tools = d.events
    .filter(
      (e): e is Extract<UIEvent, { type: "tool-call-started" }> => e.type === "tool-call-started",
    )
    .map((e) => e.toolName);
