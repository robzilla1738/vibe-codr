...
export class Engine implements EngineClient {
  ...
  #pendingQuestions = new Map<
    string,
    (answer: { answers: string[]; freeform?: string } | null) => void
  >();
  ...
  #planState: PlanState = { status: "inactive", updatedAt: Date.now() };
  ...
  constructor(opts: EngineOptions) {
    ...
    this.#orchestrationTelemetry = new OrchestrationTelemetry(this.#cwd);
    ...
    this.#jobs.configurePersistence(join(globalStateDir(this.#cwd), "monitors.json"));
    ...
    this.#session = new Session({
      ...
      askUserQuestion: (question) => this.#askUserQuestion(question),
      requestPlanMode: () => {
        if (this.#goalRunActive) {
          this.#pauseGoalRun("the agent requested plan mode");
        }
        this.#setModeGated("plan");
      },
      activityControl: (request) => this.#controlActivity(request),
  ...
  async #watchInternalEvents(): Promise<void> {
    for await (const event of this.#bus.subscribe()) {
      ...
      if (
        ...
        event.type === "orchestration-task"
      ) {
        queueMicrotask(() => this.#emitActivities());
      }
      void this.#orchestrationTelemetry.record(event);
    }
  }
  ...
  async #restoreEngineState(): Promise<void> {
    try {
      ...
      this.#goalStrategyResets = state.goalStrategyResets ?? 0;
  ...
  #resolvePlan(
    ...
    if (decision === "edit") {
      ...
      this.#setPlanState({ status: "active", updatedAt: Date.now() });
    ...
    this.#setPlanState({ ...this.#planState, status: "exit_pending", updatedAt: Date.now() });
  ...
  #setModeGated(mode: Mode): void {
    ...
    if (this.#planState.status !== "pending" && this.#planState.status !== "exit_pending") {
      this.#setPlanState({
        status: mode === "plan" ? "active" : "inactive",
        updatedAt: Date.now(),
      });
    }
  }
  ...
  #approvePlan(approvals?: "auto"): void {
    ...
    this.#lastPlan = undefined;
    this.#setPlanState({ status: "inactive", updatedAt: Date.now() });
  ...
  async bootstrap(): Promise<void> {
    ...
    this.skills.register({
      name: "handoff",
      description:
        "Prepare a concise continuation note and request a confirmed Local or Cloud session handoff",
  ...
  snapshot(): EngineSnapshot {
    return {
      ...
      planState: { ...this.#planState },
      ...(this.#questionState ? { pendingQuestion: { ...this.#questionState } } : {}),
  ...
  async #doFinalize(): Promise<void> {
    ...
    this.#settlePendingQuestions("shutdown");
  ...
  send(command: EngineCommand): void {
    switch (command.type) {
      ...
      case "abort":
        ...
        this.#settlePendingQuestions("aborted");
      ...
      case "resolve-question": {
        const resolve = this.#pendingQuestions.get(command.id);
        const question = this.#questionState;
        if (!resolve || question?.id !== command.id) break;
        const allowed = new Set(question.choices.map((choice) => choice.label));
        const answers = [...new Set(command.answers)]
          .filter((answer) => question.choices.length === 0 || allowed.has(answer))
          .slice(0, question.multiple ? 12 : 1);
        const freeform = question.allowFreeform ? command.freeform?.trim() : undefined;
        if (!answers.length && !freeform) break;
        this.#pendingQuestions.delete(command.id);
        this.#questionState = undefined;
        resolve({
          answers,
          ...(freeform ? { freeform } : {}),
        });
      ...
      case "cancel-activity":
        if (
          !this.#session.childRegistry?.cancelDetached(command.id) &&
          !this.#jobs.stopMonitor(command.id)
        )
          this.#jobs.kill(command.id);
        this.#emitActivities();
        break;
      case "shutdown":
  ...
  async #askPermission(req: {
    ...
    await this.hooks.run("permission.denied", {
      sessionId: this.#session.id,
      toolName: req.toolName,
      ...(reply.feedback ? { reason: reply.feedback } : {}),
    });
  ...
  ): Promise<{ answers: string[]; freeform?: string } | null> {
    if (!this.#interactive) return null;
    if (this.#questionState) return null;
    const signal = (this.#loopSession ?? this.#session).abortSignal;
    if (signal.aborted) return null;
    this.#questionState = question;
    let timer: ReturnType<typeof setTimeout> | undefined;
    let onAbort: (() => void) | undefined;
    return new Promise<{ answers: string[]; freeform?: string } | null>((resolve) => {
      const settle = (
        answer: { answers: string[]; freeform?: string } | null,
        reason: "aborted" | "timeout",
      ) => {
        if (!this.#pendingQuestions.delete(question.id)) return;
        this.#questionState = undefined;
        this.#bus.emit({
          type: "question-settled",
          sessionId: this.#session.id,
      ...
      onAbort = () => settle(null, "aborted");
      signal.addEventListener("abort", onAbort, { once: true });
      timer = setTimeout(() => settle(null, "timeout"), 30 * 60_000);
      this.#bus.emit({ type: "question-request", sessionId: this.#session.id, question });
    }).finally(() => {
  ...
  #settlePendingQuestions(reason: "aborted" | "shutdown"): void {
    const rows = [...this.#pendingQuestions];
    this.#pendingQuestions.clear();
    this.#questionState = undefined;
    for (const [id, resolve] of rows) {
      resolve(null);
      this.#bus.emit({ type: "question-settled", sessionId: this.#session.id, id, reason });
    }
  }
  ...
  listAgents(): AgentInfo[] {
    return [...this.#agents.values()].map((a) => ({
      ...
      ...(a.inputArtifact ? { inputArtifact: a.inputArtifact } : {}),
      ...(a.outputArtifact ? { outputArtifact: a.outputArtifact } : {}),
    }));
  }
  ...
  #beginGoalExecution(goal: string): void {
    ...
    if (!this.#goalContract) {
      this.#goalContract = freezeGoalContract(
        goal,
  ...
  #goalPlanPrompt(goal: string): string {
    return (
      ...
      "3. CONTRACT: use explicit headings `## Acceptance criteria`, `## Verification plan`, " +
      "`## Non-goals`, `## Assumed scope`, `## Implementation plan`, and `## Risks`. This becomes " +
      "the frozen completion bar used by the implementer and verifier; describe outcomes, not invented architecture.\n" +
      "4. PLAN: produce a complete step-by-step markdown checklist (`- [ ] step`) covering the " +
      ...
      "5. Do NOT modify any files, run mutating commands, or start implementing in this turn."
    );
  }
  ...
  #goalExecutePrompt(goal: string): string {
    return (
      ...
      `end to end.${this.#taskContractText()}${this.#goalContract ? formatGoalContract(this.#goalContract) : ""}\n` +
  ...
  async #maybeContinueGoal(): Promise<void> {
    ...
    if (fingerprint && fingerprint === this.#goalGapFingerprint) this.#goalStagnationCount += 1;
      ...
      this.#goalStagnationCount = fingerprint ? 1 : 0;
    }
    const strategyReset = this.#goalStagnationCount >= 2 && this.#goalStrategyResets < 2;
    if (strategyReset) {
      this.#goalStrategyResets += 1;
      this.#goalStagnationCount = 0;
      this.#notice(
        `Repeated goal gap detected — strategy reset ${this.#goalStrategyResets}/2.`,
        "warn",
      );
    }
    this.#goalContinueRounds += 1;
      ...
      () => {
        ...
        return this.#runGoalTurn(
          `The north-star goal is not yet met: "${goal}".${gaps}\n` +
            (this.#goalContract ? formatGoalContract(this.#goalContract) : "") +
            (strategyReset
              ? "\nSTRATEGY RESET: the same gap repeated. Stop repeating the prior approach. Re-read the relevant code and evidence, identify why the previous attempt failed, choose a materially different implementation strategy, and then execute it.\n"
              : "\n") +
  ...
  async #assessGoal(goal: string): Promise<{ met: boolean; gaps: string[]; reason: string }> {
    try {
      ...
      const taskBlock = tasks.length
        ...
        : "";
      const contractBlock = this.#goalContract ? formatGoalContract(this.#goalContract) : "";
      let diff = "";
      try {
  ...
  #activitySnapshot(): ActivityInfo[] {
    const shell: ActivityInfo[] = this.#jobs.snapshot().map((job) => ({
      id: job.id,
      kind: "shell",
      label: job.command,
      status:
    ...
    const monitors: ActivityInfo[] = this.#jobs.monitorSnapshot().map((monitor) => ({
      id: monitor.id,
      kind: "monitor",
      label: monitor.command,
      status: monitor.active ? "running" : "cancelled",
      ...(monitor.lastRunAt ? { startedAt: monitor.lastRunAt } : {}),
      ...(monitor.lastJobId ? { summary: `Last run: ${monitor.lastJobId}` } : {}),
    }));
    return [...shell, ...monitors, ...(this.#session.childRegistry?.activities() ?? [])];
  }
  ...
  async #controlActivity(request: {
    action: "status" | "wait" | "cancel";
    ids: string[];
    waitMode: "any" | "all";
    timeoutMs: number;
  }): Promise<{ output: string; isError?: boolean }> {
    const all = this.#activitySnapshot();
    const ids = request.ids.length ? [...new Set(request.ids)] : all.map((row) => row.id);
    if (request.action === "cancel") {
      const cancelled: string[] = [];
      for (const id of ids) {
        if (
          this.#session.childRegistry?.cancelDetached(id) ||
          this.#jobs.stopMonitor(id) ||
          this.#jobs.kill(id)
        )
          cancelled.push(id);
      }
      ...
      return cancelled.length
        ? { output: `Cancelled: ${cancelled.join(", ")}.` }
        : { output: "No selected activity was running.", isError: true };
    }
    if (request.action === "wait" && ids.length) {
      const shellIds = ids.filter((id) => id.startsWith("job_"));
      const childIds = ids.filter((id) => !id.startsWith("job_") && !id.startsWith("monitor_"));
      const waits: Promise<unknown>[] = [];
      if (shellIds.length)
        waits.push(this.#jobs.wait(shellIds, request.waitMode, request.timeoutMs));
      if (childIds.length)
        waits.push(
          this.#session.childRegistry?.waitDetached(
            childIds,
            request.waitMode,
            request.timeoutMs,
          ) ?? Promise.resolve(),
      ...
      if (waits.length) {
        await (request.waitMode === "any" ? Promise.race(waits) : Promise.allSettled(waits));
      }
    }
    const selected = this.#activitySnapshot().filter((row) => ids.includes(row.id));
    return selected.length
      ? {
          output: selected
            .map(
              (row) =>
                `${row.id} [${row.kind}/${row.status}] ${row.label}${row.summary ? `\n${row.summary}` : ""}`,
            )
            .join("\n\n"),
        }
      : {
          output: ids.length
            ? `No matching activities: ${ids.join(", ")}`
            : "No background activities.",
          ...(ids.length ? { isError: true } : {}),
        };
  }
