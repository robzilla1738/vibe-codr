...
import type { ActivityInfo, Mode } from "@vibe/shared";
...
export interface DetachedRecord {
  ...
  finishedAt?: number;
  transcript?: string;
  metrics?: ActivityInfo["metrics"];
}

interface PersistedDetached {
  id: string;
  kind: "subagent" | "tasks";
  status: "running" | "completed" | "failed" | "cancelled";
  summary: string;
...
export class ChildRegistry {
  ...
  readonly #persistedDetached = new Map<string, PersistedDetached>();
  ...
  constructor(retainMax: number, sharedCwd?: string, rootSessionId?: string) {
    this.#retainMax = Math.max(0, retainMax);
    this.#sharedCwd = sharedCwd;
    this.#statePath =
      sharedCwd && rootSessionId
        ? join(globalStateDir(sharedCwd), "sessions", rootSessionId, "children.json")
        : undefined;
    this.#restoreDetached();
  }

  #restoreDetached(): void {
    if (!this.#statePath) return;
    try {
      const rows = JSON.parse(readFileSync(this.#statePath, "utf8")) as PersistedDetached[];
      for (const row of rows) {
        // A process restart means no live controller/promise owns a formerly-running child.
        const restored: PersistedDetached =
          row.status === "running"
            ? {
                ...row,
                status: "cancelled",
                finishedAt: Date.now(),
                report: row.report ?? "Cancelled after the previous engine process exited.",
              }
            : row;
        this.#persistedDetached.set(restored.id, restored);
      }
      this.#persist();
    } catch {
      /* absent/corrupt state is an empty registry */
    }
  }
  ...
  #persist(): void {
    ...
    try {
      ...
      const live: PersistedDetached[] = [...this.#detached.values()].map((row) => ({
        id: row.id,
        kind: row.kind,
        status: row.status,
        summary: row.summary,
      ...
      const merged = new Map(this.#persistedDetached);
      for (const row of live) merged.set(row.id, row);
      writeFileSync(this.#statePath, JSON.stringify([...merged.values()].slice(-200)), "utf8");
    } catch {
  ...
  getPersistedDetached(id: string): PersistedDetached | undefined {
    return this.#persistedDetached.get(id);
  }
  ...
  async waitDetached(
    ...
    mode: "any" | "all",
    timeoutMs = 300_000,
  ): Promise<DetachedRecord[]> {
    const rows = ids
      .map((id) => this.#detached.get(id))
      .filter((row): row is DetachedRecord => !!row);
    const waits = rows.map((row) => row.promise.catch(() => undefined));
    if (waits.length) {
  ...
  activities(): ActivityInfo[] {
    const merged = new Map<string, ActivityInfo>();
    for (const row of this.#persistedDetached.values()) {
      merged.set(row.id, {
        id: row.id,
        kind: row.kind,
        ...
        ...(row.report
          ? { summary: row.report.slice(0, 2_000), outputTail: row.report.slice(-4_000) }
          : {}),
        ...(row.metrics ? { metrics: row.metrics } : {}),
      });
    }
    for (const row of this.#detached.values()) {
      merged.set(row.id, {
        id: row.id,
        kind: row.kind,
        label: row.summary || row.id,
