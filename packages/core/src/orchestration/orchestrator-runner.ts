...
function capabilityAllowlist(capabilities: NamedAgent["capabilities"]): { allow?: string[] } {
  if (!capabilities?.length) return {};
  const names = new Set<string>(["update_tasks", "ask_user_question", "read_report"]);
  if (capabilities.includes("research") || capabilities.includes("review")) {
    for (const name of [
      "read",
      "grep",
      "glob",
  ...
  if (capabilities.includes("network"))
    for (const name of ["web_search", "webfetch", "crawl_docs"]) names.add(name);
  if (capabilities.includes("code") || capabilities.includes("test"))
    for (const name of ["edit", "write", "apply_patch", "run_check", "diagnose"]) names.add(name);
  if (capabilities.includes("shell"))
    for (const name of ["bash", "job_status", "job_kill", "run_check"]) names.add(name);
  return { allow: [...names] };
}
...
export class OrchestratorRunner {
  ...
  #childRuntime = new Map<
    string,
    { transcript: string; toolCalls: number; errors: number; turns: number }
  >();
  ...
  spawnTool(): ToolDefinition<{
    ...
    return {
      ...
      execute: async ({ prompt, agent, mode, outputSchema, detach }, ctx) => {
        ...
        this.#emitStarted(child.id, prompt, named?.name);
    ...
    return {
      ...
      execute: async ({ id, message }, ctx) => {
        ...
        return this.#childRegistry.withContinueLock(id, async () => {
          let child = this.#childRegistry.lookup(id);
          if (!child) child = await this.#restorePersistedChild(id);
          if (!child) {
          ...
          this.#emitFinished(child.id, outcome.event, child);
  ...
  async #restorePersistedChild(id: string): Promise<Session | undefined> {
    const persisted = await new SessionStore(this.#handle.deps.cwd).load(id).catch(() => null);
    if (persisted?.meta.kind !== "subagent") return undefined;
    // Only restore a child owned by this root tree. This prevents one session from
    // guessing another session's child id and inheriting its private transcript.
    if (persisted.meta.parentSessionId && persisted.meta.parentSessionId !== this.#handle.id)
      return undefined;
    const named = persisted.meta.agentName
      ? this.#handle.deps.agents?.get(persisted.meta.agentName)
      : undefined;
    const extraSystem: string[] = [];
    if (named?.system) extraSystem.push(named.system);
    if (named) {
      extraSystem.push(
        `WORKER CONTRACT:\nPersona: ${named.persona ?? "focused coding agent"}\n` +
          `Capabilities: ${(named.capabilities ?? []).join(", ") || "inherited"}\n` +
          (named.inputArtifact ? `Required input: ${named.inputArtifact}\n` : "") +
          (named.outputArtifact ? `Required output: ${named.outputArtifact}\n` : "") +
          (named.completionRequirement
            ? `Completion requirement: ${named.completionRequirement}`
            : ""),
      );
    }
    if (this.#handle.deps.repoMap) {
      extraSystem.push(
        `REPO SYMBOL MAP (refreshed for this continuation):\n${this.#handle.deps.repoMap}`,
      );
    }
    const usage = persisted.meta.usage;
    const child = this.#handle.fork({
      id: persisted.meta.id,
      model: persisted.meta.model,
      mode: persisted.meta.mode,
      goal: persisted.meta.goal,
      initialModelMessages: persisted.modelMessages,
      initialHistory: persisted.history,
      ...(persisted.meta.tasks ? { initialTasks: persisted.meta.tasks } : {}),
      ...(usage
        ? {
            initialUsage: usage,
            initialCostUSD: usage.costUSD ?? 0,
            initialActualCostUSD: usage.actualCostUSD ?? 0,
            initialCostEstimated: usage.costEstimated ?? false,
          }
        : {}),
      ...(persisted.meta.lastInputTokens
        ? { initialLastInputTokens: persisted.meta.lastInputTokens }
        : {}),
      ...(persisted.meta.sources?.length ? { initialSources: persisted.meta.sources } : {}),
      ...(persisted.meta.offloaded?.length ? { initialOffloaded: persisted.meta.offloaded } : {}),
      ...(extraSystem.length ? { extraSystem } : {}),
      ...(named?.tools || named?.denyTools || named?.capabilities
        ? {
            toolFilter: {
              ...(named?.tools ? { allow: named.tools } : capabilityAllowlist(named?.capabilities)),
      ...
      ...(named?.completionRequirement
        ? {
            completionRequirement: {
              text: named.completionRequirement,
              ...(named.completionTool ? { tool: named.completionTool } : {}),
              maxRetries: 1,
            },
          }
        : {}),
      store: new SessionStore(this.#handle.deps.cwd),
      sessionKind: "subagent",
      parentSessionId: this.#handle.id,
      ...(persisted.meta.agentName ? { agentName: persisted.meta.agentName } : {}),
    ...
    this.#handle.deps.bus.emit({
      type: "notice",
      level: "info",
      message: `Restored subagent \`${id}\` with its persisted conversation context.`,
    });
    return child;
  }
  ...
  checkTaskTool(): ToolDefinition<{
    ...
    return {
      ...
      inputSchema: z.object({
        id: z.string().optional().describe("One background spawn id."),
        ids: z.array(z.string()).optional().describe("Several ids for wait-any/wait-all."),
        wait: z.boolean().optional().describe("Wait for completion instead of only polling."),
        waitMode: z.enum(["any", "all"]).optional(),
        timeoutMs: z.number().int().min(0).max(3_600_000).optional(),
        cancel: z.boolean().optional().describe("Cancel the selected running activity."),
      }),
      readOnly: true,
      concurrencySafe: true,
      execute: async ({ id, ids, wait, waitMode, timeoutMs, cancel }) => {
        const selected = [...new Set([...(ids ?? []), ...(id ? [id] : [])])];
        if (!selected.length) return { output: "Pass `id` or `ids`.", isError: true };
        if (cancel) {
          const cancelled = selected.filter((value) => this.#childRegistry.cancelDetached(value));
          return {
            output: cancelled.length
              ? `Cancelled: ${cancelled.join(", ")}.`
              : "No selected activity was running.",
            ...(cancelled.length ? {} : { isError: true }),
          };
        }
        if (wait)
          await this.#childRegistry.waitDetached(selected, waitMode ?? "all", timeoutMs ?? 300_000);
        if (selected.length > 1) {
          return {
            output: selected
              .map((value) => {
                const row = this.#childRegistry.getDetached(value);
                const old = this.#childRegistry.getPersistedDetached(value);
                return row
                  ? `${value}: ${row.status}`
                  : old
                    ? `${value}: ${old.status}`
        ...
        const rec = this.#childRegistry.getDetached(selectedId);
        if (!rec) {
          const persisted = this.#childRegistry.getPersistedDetached(selectedId);
          if (persisted)
            return {
              output: `Background spawn \`${selectedId}\` ${persisted.status}:\n${capSubagentOutput(persisted.report ?? "(no report)")}`,
              ...(persisted.isError ? { isError: true } : {}),
            };
          return {
            output: `No background spawn with id "${selectedId}" in this session.`,
            isError: true,
          };
        }
        if (rec.status === "running") {
          return {
            output: `Background spawn \`${selectedId}\`${rec.summary ? ` (${rec.summary})` : ""} is still running.`,
          };
        }
        return {
          output: `Background spawn \`${selectedId}\` ${rec.status}:\n${capSubagentOutput(rec.report ?? "(no report)")}`,
  ...
  #forkChild(
    ...
    if (
      counter &&
      childMode !== "plan" &&
      counter.used >= this.#handle.deps.config.subagent.maxTotal
    )
      return null;
    ...
    if (named) {
      extraSystem.push(
        `WORKER CONTRACT:\n` +
          ...
          (named.outputArtifact ? `Required output: ${named.outputArtifact}\n` : "") +
          (named.completionRequirement
            ? `Completion requirement: ${named.completionRequirement}\nYou MUST satisfy this requirement before claiming completion.`
            : ""),
      );
    }
    if (this.#handle.deps.repoMap) {
    ...
    const child = this.#handle.fork({
      ...
      ...(named?.completionRequirement
        ...
        : {}),
      store: new SessionStore(cwdOverride ?? this.#handle.deps.cwd),
      sessionKind: "subagent",
      parentSessionId: this.#handle.id,
      ...(named?.name ? { agentName: named.name } : {}),
    });
    this.#tapChildActivity(child.id, childBus);
    return child;
  }
  ...
  #tapChildActivity(childId: string, childBus: EventBusImpl): void {
    ...
    let lastEmit = 0;
    const runtime = this.#childRuntime.get(childId) ?? {
      transcript: "",
      toolCalls: 0,
      errors: 0,
      turns: 0,
    };
    this.#childRuntime.set(childId, runtime);
    // Consume the child's stream; ends when the bus closes (stop() below).
    void (async () => {
      for await (const e of sub) {
        let label: string | undefined;
        let transcriptDelta: string | undefined;
        if (e.type === "assistant-text-delta" || e.type === "reasoning-delta")
          transcriptDelta = e.delta;
        else if (e.type === "tool-call-started") {
          label = activityLabel(e.toolName, e.input);
          runtime.toolCalls++;
          transcriptDelta = `\n[tool] ${label}\n`;
        } else if (e.type === "tool-call-finished" && e.isError) runtime.errors++;
        else if (e.type === "turn-finished") runtime.turns++;
        else if (e.type === "file-changed") label = `✎ ${e.path}`;
        if (transcriptDelta)
          runtime.transcript = (runtime.transcript + transcriptDelta).slice(-256_000);
        if (!label && transcriptDelta) label = "Responding…";
        ...
        this.#handle.deps.bus.emit({
          ...
          metrics: { turns: runtime.turns, toolCalls: runtime.toolCalls, errors: runtime.errors },
        });
      }
    })();
  ...
  #emitStarted(id: string, prompt: string, agent?: string): void {
    this.#handle.deps.bus.emit({
      type: "subagent-started",
      sessionId: this.#handle.id,
      subagentId: id,
    ...
    void this.#handle.deps.hooks?.run("subagent.start", {
      sessionId: this.#handle.id,
      subagentId: id,
      prompt,
      ...(agent ? { agent } : {}),
  ...
  #emitFinished(id: string, result: string, child?: Session): void {
    const runtime = this.#childRuntime.get(id);
    ...
    this.#handle.deps.bus.emit({
      ...
      result,
      finishedAt: Date.now(),
      ...(runtime?.transcript ? { transcript: runtime.transcript } : {}),
      metrics: {
        ...(runtime
          ? { turns: runtime.turns, toolCalls: runtime.toolCalls, errors: runtime.errors }
          : {}),
        ...(usage ? { inputTokens: usage.inputTokens, outputTokens: usage.outputTokens } : {}),
      },
    });
    void this.#handle.deps.hooks?.run("subagent.stop", {
      sessionId: this.#handle.id,
      subagentId: id,
      result,
      failed: child?.lastError !== null || child?.interrupted === true,
    });
    this.#childRuntime.delete(id);
  }
  ...
  async #runStructured(
    ...
    try {
      while (attempts < maxAttempts) {
        ...
        if (result.ok) {
          this.#emitFinished(child.id, result.json, child);
          this.#retainCompleted(child);
      ...
      const msg =
        ...
        `Validation errors:\n${errors.map((e) => `- ${e}`).join("\n")}\n\n` +
        `Raw final message:\n${raw || "(empty)"}`;
      this.#emitFinished(child.id, `schema mismatch after ${attempts} attempt(s)`, child);
      this.#retainCompleted(child);
  ...
  async #runChildToCompletion(
    ...
    try {
      let result = await this.#runChildOnce(child, prompt, parentSignal, suspendParentSlot);
      const requirement = child.completionRequirement;
      if (
        !result.timedOut &&
        !result.aborted &&
        !child.lastError &&
        requirement?.tool &&
        !child.hasUsedTool(requirement.tool)
      ) {
        const retry = await this.#runChildOnce(
          child,
          `Completion requirement was not satisfied: ${requirement.text}\nYou did not call the required tool \`${requirement.tool}\`. Do that now, inspect the real result, then provide your final report.`,
          parentSignal,
          suspendParentSlot,
        );
        result = {
          timedOut: result.timedOut || retry.timedOut,
          aborted: result.aborted || retry.aborted,
        };
      }
      return result;
    } finally {
  ...
  #childOutcome(
    ...
    const requirement = child.completionRequirement;
    if (requirement?.tool && !child.hasUsedTool(requirement.tool)) {
      return {
        text:
          `Subagent did not satisfy its completion requirement after the bounded recovery turn: ${requirement.text}\n` +
          `Required tool not called: ${requirement.tool}.`,
        isError: true,
        event: `failed completion requirement: ${requirement.tool}`,
      };
    }
    const text =
      partial ||
