...
export function buildEnterPlanModeTool(
  ...
  return {
    ...
    description:
      "Request a transition into read-only plan mode when the task needs investigation and user approval before implementation.",
    inputSchema: z.object({}),
    readOnly: true,
    concurrencySafe: false,
    modes: ["execute"],
    execute: async () => {
      if (!handle.deps.requestPlanMode)
        return { output: "Plan-mode transition is unavailable.", isError: true };
      handle.deps.requestPlanMode();
      return {
        output:
          "Switched to read-only plan mode. Investigate, then call present_plan and wait for approval.",
      };
    },
  };
}

export function buildActivityTool(handle: SessionToolsHandle): ToolDefinition<{
  ...
  return {
    name: "activity",
    description:
      "Unified control for background shell jobs, detached subagents/task batches, and monitors. " +
      "List/status activities, wait for any/all selected ids, or cancel selected ids.",
    inputSchema: z.object({
    ...
    concurrencySafe: false,
    modes: ["execute"],
    execute: async ({ action, ids, waitMode, timeoutMs }) => {
      if (!handle.deps.activityControl)
        return { output: "Activity control is unavailable.", isError: true };
      return handle.deps.activityControl({
        action,
        ids: ids ?? [],
...
export function buildAskUserQuestionTool(handle: SessionToolsHandle): ToolDefinition<{
  question: string;
  header?: string;
  ...
  return {
    ...
    description:
      "Ask one concise, structured clarification only when the answer materially changes the work. " +
      "Provide 2–4 mutually exclusive choices when possible; allow free-form when choices cannot cover it. " +
      "The call blocks until the user answers or cancels.",
    inputSchema: z.object({
    ...
    readOnly: true,
    concurrencySafe: false,
    execute: async ({ question, header, choices, multiple, allowFreeform }) => {
      if (!handle.deps.askUserQuestion) {
        return {
          output:
            "Structured questions require an interactive UI; continue with a clearly stated assumption.",
          isError: true,
        };
      }
      const request: StructuredQuestion = {
      ...
      const result = await handle.deps.askUserQuestion(request);
      if (!result)
        return {
          output: "The question was cancelled; continue only with a clearly stated assumption.",
          isError: true,
        };
      return {
        output:
          `User answered: ${result.answers.length ? result.answers.join(", ") : "(no choice)"}` +
          (result.freeform ? `\nAdditional context: ${result.freeform}` : ""),
      };
    },
  };
}
  ...
  return {
    ...
    execute: async ({ target, provider, instruction }, ctx) => {
      if (target === "cloud" && !provider) {
        return {
          output: "Choose provider e2b or vercel before requesting a cloud handoff.",
          isError: true,
        };
      }
      ctx.emit({
