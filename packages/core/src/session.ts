...
export interface SessionDeps {
  ...
  interactive?: boolean;
  /** Interactive structured clarification bridge owned by the engine/UI. */
  askUserQuestion?: (
    question: StructuredQuestion,
  ) => Promise<{ answers: string[]; freeform?: string } | null>;
  completionRequirement?: { text: string; tool?: string; maxRetries?: number };
  requestPlanMode?: () => void;
  sessionKind?: "root" | "subagent";
  parentSessionId?: string;
  agentName?: string;
  activityControl?: (request: {
...
export class Session {
  ...
  get completionRequirement(): SessionDeps["completionRequirement"] {
    return this.#deps.completionRequirement;
  }

  hasUsedTool(name: string): boolean {
    return this.#usedTools.has(name);
  }
  ...
  async run(
    ...
    try {
      ...
      if (this.#deps.interactive && this.depth === 0 && this.#deps.askUserQuestion) {
        tools.ask_user_question = toAISDKTool(
          buildAskUserQuestionTool(this.#handle),
          base,
          serialize,
        );
      }
      if (
        ...
        this.depth === 0 &&
        this.mode !== "plan" &&
        this.#deps.requestPlanMode
      ) {
        tools.enter_plan_mode = toAISDKTool(buildEnterPlanModeTool(this.#handle), base, serialize);
      }
      if (this.#deps.interactive && this.depth === 0 && this.#deps.activityControl) {
        tools.activity = toAISDKTool(buildActivityTool(this.#handle), base, serialize);
      }
      ...
      if (this.#streamStalled) {
        ...
        await this.#deps.hooks?.run("turn.failure", {
          sessionId: this.id,
  ...
  async compact(): Promise<void> {
    await this.#deps.hooks?.run("compact.before", { sessionId: this.id });
  ...
  async #maybeCompact(model: LanguageModel, force: boolean): Promise<void> {
    ...
    await this.#deps.hooks?.run("compact.after", { sessionId: this.id, freedTokens: result.freed });
    if (result.overrun) {
      this.#deps.bus.emit({
        type: "notice",
        level: "warn",
      ...
      await store.save(
        {
          ...
          mode: this.mode,
          goal: this.goal,
          kind: this.#deps.sessionKind ?? "root",
          ...(this.#deps.parentSessionId ? { parentSessionId: this.#deps.parentSessionId } : {}),
