...
import { ensureSubscriptionToken, ProviderAuthManager, subscriptionFetch, xaiDevicePollDecision } from "./oauth.ts";
...
test("Codex browser OAuth verifies callback state, persists 0600 tokens, and reports connected", async () => {
  ...
  const access = unsignedJwt({ exp: Math.floor(Date.now() / 1000) + 3600, email: "dev@example.com", chatgpt_account_id: "acct_1" });
  globalThis.fetch = (async (input: string | URL | Request) => {
    expect(input.toString()).toBe("https://auth.openai.com/oauth/token");
    return new Response(JSON.stringify({ access_token: access, refresh_token: "refresh-1", expires_in: 3600 }), {
      status: 200,
  ...
  await new Promise<void>((resolve, reject) => {
    get(`http://localhost:1455/auth/callback?code=ok&state=${authorize.searchParams.get("state")}`, (response) => {
      response.resume();
  ...
  expect(store.providers["openai-codex"]).toMatchObject({ access, refresh: "refresh-1", accountId: "acct_1" });
...
test("subscription fetch replaces bearer auth and adds the Codex account header", async () => {
  ...
  globalThis.fetch = (async () => new Response(JSON.stringify({
    access_token: unsignedJwt({ exp: Math.floor(Date.now() / 1000) + 3600, chatgpt_account_id: "acct_2" }),
...
test("xAI device polling follows pending, slow-down, expiry, and denial semantics", () => {
  expect(xaiDevicePollDecision("authorization_pending", 5000)).toEqual({ action: "wait", intervalMs: 5000 });
  expect(xaiDevicePollDecision("slow_down", 5000)).toEqual({ action: "wait", intervalMs: 10_000 });
  expect(xaiDevicePollDecision("expired_token", 5000)).toMatchObject({ action: "fail", message: expect.stringContaining("expired") });
  expect(xaiDevicePollDecision("access_denied", 5000)).toMatchObject({ action: "fail", message: expect.stringContaining("denied") });
});
...
test("xAI refresh rotates the refresh token in the user-only store", async () => {
  ...
  await writeFile(process.env.VIBE_AUTH_PATH, JSON.stringify({
    version: 1,
  ...
  expect(await ensureSubscriptionToken("xai-oauth")).toMatchObject({ access: "new-access", refresh: "new-refresh" });
  const stored = JSON.parse(await readFile(process.env.VIBE_AUTH_PATH, "utf8"));
  expect(stored.providers["xai-oauth"]).toMatchObject({ access: "new-access", refresh: "new-refresh" });
});
