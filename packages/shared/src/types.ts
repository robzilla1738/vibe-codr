...
export interface AgentInfo {
  ...
  mode: Mode;
  /** Human working style, separate from operational capability. */
  persona?: string;
  /** Explicit capability profile used to constrain tools and route work. */
  capabilities?: AgentCapability[];
  /** Artifact contracts make handoffs inspectable instead of prose-only. */
...
/** Frozen at the plan→execute seam; every later verifier sees the same bar. */
...
/** Persisted plan lifecycle, including the approval payload needed to rehydrate UI. */
...
/** A model-authored, UI-resolved clarification request. */
...
/** One status shape for shell jobs, detached agents, task batches, and monitors. */
...
export interface GoalRunInfo {
  ...
  met: boolean;
  /** The immutable completion bar captured before implementation begins. */
  contract?: GoalContract;
  /** Repeated-gap detector state, useful for honest progress surfaces. */
...
export interface EngineSnapshot {
  ...
  goalRun?: GoalRunInfo;
  /** Full plan lifecycle so a pending approval survives process/UI restart. */
  planState?: PlanState;
