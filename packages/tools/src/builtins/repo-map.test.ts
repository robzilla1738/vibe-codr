...
test("queryScopeGraph returns definitions and importing-file references", async () => {
  const dir = mkdtempSync(join(tmpdir(), "vibe-scope-graph-"));
  mkdirSync(join(dir, "src"), { recursive: true });
  writeFileSync(join(dir, "src", "core.ts"), "export function durablePlan() {}\n");
  writeFileSync(
    join(dir, "src", "consumer.ts"),
    "import { durablePlan } from './core';\nexport const run = durablePlan;\n",
  );
  const graph = await queryScopeGraph(dir, "durablePlan");
  expect(
    graph.definitions.some(
      (item) => item.file === "src/core.ts" && item.declaration.includes("function durablePlan"),
    ),
  ).toBe(true);
  expect(graph.references).toContain("src/consumer.ts");
});
