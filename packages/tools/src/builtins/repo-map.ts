...
const Input = z.object({
  ...
  symbol: z
    .string()
    .optional()
    .describe("Find likely definitions and importing-file references for one symbol."),
});
...
const loadedPersistentCaches = new Set<string>();

function graphCachePath(cwd: string): string {
  const hash = createHash("sha256").update(normalize(cwd)).digest("hex").slice(0, 16);
  const root = process.env.VIBE_STATE_DIR || join(homedir(), ".vibe", "state");
  return join(root, hash, "scope-graph.json");
}

function loadPersistentGraph(cwd: string): Map<string, FileEntry> {
  if (loadedPersistentCaches.has(cwd)) return mapCache.get(cwd) ?? new Map();
  loadedPersistentCaches.add(cwd);
  try {
    const parsed = JSON.parse(readFileSync(graphCachePath(cwd), "utf8")) as {
      version?: number;
      files?: [string, FileEntry][];
    };
    if (parsed.version === 1 && Array.isArray(parsed.files)) {
      const cache = new Map(parsed.files);
      mapCache.set(cwd, cache);
      return cache;
    }
  } catch {
    /* cold cache */
...
function persistGraph(cwd: string, cache: Map<string, FileEntry>): void {
  try {
    const path = graphCachePath(cwd);
    mkdirSync(dirname(path), { recursive: true });
    writeFileSync(
      path,
      JSON.stringify({ version: 1, cwd: normalize(cwd), updatedAt: Date.now(), files: [...cache] }),
      "utf8",
    );
  } catch {
...
export async function queryScopeGraph(
  cwd: string,
  symbol: string,
  path?: string,
): Promise<{ definitions: { file: string; declaration: string }[]; references: string[] }> {
  // Refresh the incremental index first; unchanged files are served from the
  // persistent cache and only changed mtimes are re-read.
  await buildRepoMap(cwd, { ...(path ? { path } : {}), maxFiles: DEFAULT_MAX_FILES });
  const cache = loadPersistentGraph(cwd);
  const all = await listFiles(cwd, path);
  const known = new Set(all);
  const needle = symbol.trim().toLowerCase();
  const definitions: { file: string; declaration: string }[] = [];
  const definitionFiles = new Set<string>();
  for (const [file, entry] of cache) {
    for (const declaration of entry.symbols) {
      const declared =
        /(?:^|\s)(?:function|class|interface|type|enum|const|let|var|def|func|struct|trait|protocol)\s+([A-Za-z_$][\w$]*)/.exec(
          declaration,
        )?.[1];
      if (declared?.toLowerCase() !== needle) continue;
      definitions.push({ file, declaration });
  ...
  for (const [file, entry] of cache) {
    for (const spec of entry.imports) {
      ...
      if (target && definitionFiles.has(target) && file !== target) references.add(file);
    }
  }
  return { definitions: definitions.slice(0, 100), references: [...references].slice(0, 200) };
}
...
export async function buildRepoMap(
  ...
  const cacheKey = cwd;
  const cache = loadPersistentGraph(cacheKey);
...
export const repoMapTool: ToolDefinition<z.infer<typeof Input>> = {
  ...
  async execute({ path, maxFiles, symbol }, ctx) {
    if (symbol?.trim()) {
      const graph = await queryScopeGraph(ctx.cwd, symbol, path);
      if (!graph.definitions.length) {
        return {
          output: `No indexed definition found for symbol \`${symbol.trim()}\`${path ? ` under ${path}` : ""}.`,
        };
      }
      return {
        output:
          `Scope graph for \`${symbol.trim()}\`:\n\nDefinitions:\n` +
          graph.definitions.map((item) => `- ${item.file} — ${item.declaration}`).join("\n") +
          `\n\nImporting-file references:\n` +
          (graph.references.length
            ? graph.references.map((file) => `- ${file}`).join("\n")
            : "(none indexed)"),
      };
    }
