...
test("rewritePackageLockVersion updates both npm root version fields", () => {
  const lock = JSON.stringify({
    name: "vibe-codr-desktop",
    version: "0.1.0",
    packages: { "": { name: "vibe-codr-desktop", version: "0.1.0" }, "node_modules/x": { version: "9.9.9" } },
  });
  const out = JSON.parse(rewritePackageLockVersion(lock, "0.6.0"));
  expect(out.version).toBe("0.6.0");
  expect(out.packages[""].version).toBe("0.6.0");
  expect(out.packages["node_modules/x"].version).toBe("9.9.9");
});
