...
execFileSync("docker", [
  ...
  [
    ...
    "  host.stdin.write(`${JSON.stringify({ op: 'bootstrap', cwd: '/tmp/source' })}\\n`);",
    ...
    "    if (frame.channel === 'engine' && frame.payload?.type === 'ready') { if (frame.payload.sessionId !== expectedSessionId) { clearTimeout(timer); reject(new Error(`cloud resume mismatch: ${frame.payload.sessionId}`)); return; } clearTimeout(timer); socket.close(); resolve(); }",
