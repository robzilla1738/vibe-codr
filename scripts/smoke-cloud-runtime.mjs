...
execFileSync("docker", [
  ...
  [
    "set -eu",
    "mkdir -p /tmp/runtime",
    `tar -xzf '/artifacts/${archive}' -C /tmp/runtime`,
    "cd /tmp/runtime",
    ...
    "  try { const response = await fetch('http://127.0.0.1:8787/health', { headers: { authorization: `Bearer ${token}` } }); if (response.ok) { process.stdout.write('cloud runtime offline smoke ok\\n'); process.exit(0); } last = `HTTP ${response.status}`; } catch (error) { last = error.message; }",
    "  await new Promise((resolve) => setTimeout(resolve, 250));",
    "}",
    "throw new Error(`cloud agent health failed: ${last}\\n${readFileSync('/tmp/cloud-agent.log', 'utf8')}`);",
    "NODE",
    "kill -0 \"$AGENT_PID\"",
    "kill -TERM \"$AGENT_PID\"",
    "set +e",
